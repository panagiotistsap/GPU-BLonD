...
from blond.gpu.gpu_butils_wrap import gpu_copy_d2d,first_kernel_tracker,second_kernel_tracker,\
...
def gpu_track(self):
    ...
    if self.phi_noise is not None:
        if self.noiseFB is not None:
            first_kernel_tracker(self.dev_phi_rf, self.noiseFB.x, self.dev_phi_noise, self.dev_phi_rf.shape[0], turn)
            # self.phi_rf[:, turn] += \
            #     self.noiseFB.x * self.phi_noise[:, turn]
        else:
            first_kernel_tracker(self.dev_phi_rf, 1.0, self.dev_phi_noise, self.dev_phi_rf.shape[0], turn)
            # self.phi_rf[:, turn] += \
            #     self.phi_noise[:, turn]
