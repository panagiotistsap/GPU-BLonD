...
extern "C"
__global__ void drift_simple(float *beam_dt,
                             const float *beam_dE,
                             const float T0, const float length_ratio,
                             const float eta0, const float beta,
                             const float energy,
                             const int n_macroparticles)
{
    float T = T0 * length_ratio;
    float coeff = T * (eta0 / (beta * beta * energy));
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
...
extern "C"
__global__ void drift_legacy_0(float *beam_dt,
                               const float *beam_dE,
                               const float T,
                               const float eta0,
                               const int n_macroparticles)
...
extern "C"
__global__ void drift_legacy_1(float *beam_dt,
                               const float *beam_dE,
                               const float T,
...
extern "C"
__global__ void drift_legacy_2(float *beam_dt,
                               const float *beam_dE,
                               const float T,
                               const float eta0, const float eta1, const float eta2,
                               const int n_macroparticles)
{
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
    float my_beam_dE;
...
extern "C"
__global__ void drift_else(float *beam_dt,
                           const float *beam_dE,
                           const float invbetasq,
                           const float invenesq,
                           const float T,
                           const float alpha_zero,
                           const float alpha_one,
...
extern "C"
__global__ void set_zero_complex(pycuda::complex<float> *x, long n)
{
    unsigned tid = threadIdx.x;
    unsigned total_threads = gridDim.x * blockDim.x;
    unsigned cta_start = blockDim.x * blockIdx.x;
...
extern "C"
__global__ void set_zero_complex_range(pycuda::complex<float> *x , long start, long stop, long step)
{
    unsigned tid = threadIdx.x;
    unsigned total_threads = gridDim.x * blockDim.x;
    unsigned cta_start = blockDim.x * blockIdx.x;
