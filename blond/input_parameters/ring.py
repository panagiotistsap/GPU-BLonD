...
class Ring(object):
    ...
    def __init__(self, ring_length, alpha_0, synchronous_data, Particle,
        ...
        self.ring_length = np.array(ring_length, ndmin=1, dtype=np.precision.real_t)
        ...
        self.momentum = self.momentum.astype(dtype=np.precision.real_t, order='C', copy=False)
        ...
        if alpha_2 is not None:
            ...
            # to avoid high order momentum compaction programs filled
            # with zeros (should be propagated in RFStation.__init__())
            self.alpha_2 = np.zeros(self.alpha_0.shape, dtype=np.precision.real_t)
    ...
    def eta_generation(self):
        ...
        for i in range(self.alpha_order+1, 3):
            setattr(self, "eta_%s" % i, np.zeros([self.n_sections,
                                                  self.n_turns+1]), dtype=np.precision.real_t)

    def _eta0(self):
        """ Function to calculate the zeroth order slippage factor eta_0 """

        self.eta_0 = np.empty([self.n_sections, self.n_turns+1], dtype=np.precision.real_t)
    ...
    def _eta1(self):
        ...
        self.eta_1 = np.empty([self.n_sections, self.n_turns+1], dtype=np.precision.real_t)
        for i in range(0, self.n_sections):
    ...
    def _eta2(self):
        ...
        self.eta_2 = np.empty([self.n_sections, self.n_turns+1], dtype=np.precision.real_t)
        for i in range(0, self.n_sections):
